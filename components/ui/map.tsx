...
function MapDrawControl({
  ...
  // FIX: Wrapped in useCallback and added dependencies
...
function MapDrawShapeButton<T extends Draw.Feature>({
  ...
  if (!drawContext) {
    throw new Error(
      "MapDrawShapeButton must be used within MapDrawControl"
    );
  }
  const { L } = useLeaflet();
  const map = useMap();
...
function MapDrawActionButton<T extends EditToolbar.Edit | EditToolbar.Delete>({
  ...
  useEffect(() => {
    ...
    // FIX: Added controlRef to dependencies
...
function useLeaflet() {
  ...
  useEffect(() => {
    ...
    // FIX: Replaced require() with dynamic imports
    (async () => {
      const leafletModule = await import("leaflet");
      const drawModule = await import("leaflet-draw");
      setL(leafletModule);
      setLeafletDraw(drawModule);
    })();
  }, [L, LeafletDraw]);
