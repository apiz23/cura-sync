...
async function migrateProfileId(oldId: string, newId: string) {
    ...
    if (oldPatientProfile) {
        ...
        if (upsertErr) {
            console.error(`[sync] Failed to upsert patient profile for ${newId}:`, upsertErr);
            // Abort migration — do NOT delete old row on failure
        ...
        console.error(`[sync] Failed to read patient profile for ${oldId}:`, readErr);
    }
